...
def ape_plotting():
    ...
    plot_data.append(go.Scatter(x=wavelength, y=selected_data, mode='markers', name=r'$\small\textrm{Col. ' + str(data_column) + '}$'))
    ...
    plot_data.append(go.Scatter(x=x_range, y=y_pred2, mode='lines', name=r"$\small x$"))
    ...
    plot_data.append(go.Scatter(x=x_range, y=y_pred3, mode='lines', name=r"\small $x^2$"))
    # print(r_squared3)
    ...
    plot_data.append(go.Scatter(x=x_range, y=y_pred4, mode='lines', name=r"$\small x^3$"))
    # print(r_squared3)
    ...
    plot_data.append(go.Scatter(x=x_range, y=y_pred5, mode='lines', name=r"$\small x^4$"))
    # print(r_squared3)

    layout = go.Layout(title=r'$\Large\textrm{Fitting}$',
                       yaxis={'title': r'$\small\textrm{Column ' + str(data_column) + '}$', 'automargin': True, 'tickformat':'none'},
                       xaxis={'title': r'$\small\textrm{Wavelength (cm}^{-1}\textrm{)}$', 'automargin': True, 'tickformat':'none'},
    ...
    table = go.Figure(data=[go.Table(header=dict(values=['Order','A', 'B', 'C', 'D', 'E', 'R^2']),
