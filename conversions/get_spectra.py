# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 11:07:53 2022
...
def LengthCheck(array, X, xcm):
    if array.shape[1] != np.array([X]).shape[
        1]:  # Sometimes the number of indices between minWN and maxWN varies by 1. This fixes that
...
def Apodization(pos_corr, intfgm_corr):
    ...
    mask = [i for i in np.arange(1, len(INTfgm) - 2, 1) if
            INTfgm[i] > INTfgm[i + 1] and INTfgm[i] > INTfgm[i - 1]]  # Find all local maxima
    INT_smooth = savitzky_golay(INTfgm[mask], 11, 3)
    POS_smooth = pos_corr[mask]
    centerBurst = [i for i in np.arange(0, len(INT_smooth) - 1, 1) if
                   INT_smooth[i] > np.max(INT_smooth) / 2]  # Find all points greater than half the maximum
    centerI = [i for i in np.arange(0, len(INT_smooth) - 1, 1) if
               INT_smooth[i] == np.max(INT_smooth)]  # Find the index of the maximum
...
def get_spectrum_from_interferogram(samp_data, ref_data, params):
    ...
    for j in range(int(r.shape[1]-1)):
        ...
        FTsig_ref,FTfreq_ref = FourierTransform(pos_corr,intfgm_corr,3) #Fourier transform

        mask = [i for i in range(len(FTfreq_ref)) if minWN < FTfreq_ref[i] and FTfreq_ref[i] < maxWN] #Find desired frequency range
        xcm = FTfreq_ref[mask] #Restrict data to only desired frequency range
        X = FTsig_ref[mask]

        if len(refArray)==0: #Best way I could think of to make the shape of rafArray work for vstack
            refArray = np.array([xcm])
        refArray,X,xcm = LengthCheck(refArray,X,xcm)

        #Phase correction
        p = np.poly1d(np.polyfit(xcm,np.angle(X),2)) #Fitting with a line
        X = X*np.exp(-1j*p(xcm))
        refArray = np.vstack((refArray,X))

    refArray = np.transpose(refArray) #Now all the reference arrays are in a matrix with xcm as the first column and the fourier transformed data in the next columns
    refAvg = np.zeros((refArray.shape[0],2),dtype=np.complex_)
    refAvg[:,0] = refArray[:,0]
    for i in range(refArray.shape[0]):
        refAvg[i,1] = np.mean(refArray[i,1:])
    ...
    for j in range(int(r.shape[1] - 1)):
        ...
        #                if refAvg.shape[0] < sampArray.shape[1]:
        #                    sampArray = sampArray[:,:-1]
        #                else:
        #                    refAvg = refAvg[:-1,:]
